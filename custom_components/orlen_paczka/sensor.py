"""Sensor platform for the ORLEN Paczka parcel tracker integration."""
...
from . import ORLENPaczkaConfigEntry
from .const import DOMAIN, ParcelStatus
from .coordinator import ORLENPaczkaCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: ORLENPaczkaConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up ORLEN Paczka sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        ...
        ORLENPaczkaAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            ORLENPaczkaParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(ORLENPaczkaNextDeliverySensor(coordinator, entry))
    entities.append(ORLENPaczkaDeliveredParcelsSensor(coordinator, entry))
    entities.append(ORLENPaczkaLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class ORLENPaczkaAwaitingPickupSensor(
    CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: ORLENPaczkaCoordinator, entry: ConfigEntry) -> None:
...
class ORLENPaczkaIncomingParcelsSensor(
    CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: ORLENPaczkaCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                ORLENPaczkaParcelSensor(self.coordinator, self._entry, barcode)
...
class ORLENPaczkaParcelSensor(CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked ORLEN Paczka parcel."""
    ...
    def __init__(
        self, coordinator: ORLENPaczkaCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class ORLENPaczkaNextDeliverySensor(
    CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: ORLENPaczkaCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class ORLENPaczkaDeliveredParcelsSensor(
    CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity
):
    """Recently delivered tracked ORLEN Paczka parcels."""
...
class ORLENPaczkaLastUpdateSensor(
    CoordinatorEntity[ORLENPaczkaCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when ORLEN Paczka was last polled successfully."""
