"""Config flow for the ORLEN Paczka parcel tracker integration."""
...
def normalize_tracking_code(value: str) -> str:
    ...
    Stripping visual separators lets users paste a numeric code from a shipping
    notification without changing its identity.
    """
...
class ORLENPaczkaConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the ORLEN Paczka integration."""
    ...
    ) -> ORLENPaczkaOptionsFlowHandler:
        """Return the options flow handler."""
        return ORLENPaczkaOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the ORLEN Paczka hub — single instance, no input needed.
        ...
        the ``orlen_paczka.track_parcel`` service or a dashboard button.
...
class ORLENPaczkaOptionsFlowHandler(OptionsFlow):
    ...
    async def async_step_parcels(
        ...
        if user_input is not None:
            ...
            if any(code.startswith("AD") for code in codes):
                errors["base"] = "allegro_delivery"
            elif any(not valid_tracking_code(code) for code in codes):
