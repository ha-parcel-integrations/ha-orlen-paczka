"""Coordinator for the ORLEN Paczka parcel tracker integration.
...
from .api import ORLENPaczkaApiClient, ORLENPaczkaApiError
from .const import (
...
class ORLENPaczkaCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (ORLENPaczkaApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, ORLENPaczkaApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("ORLEN Paczka fetch failed: %s", result)
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "ORLEN Paczka rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("ORLEN Paczka unreachable for all tracked parcels")
