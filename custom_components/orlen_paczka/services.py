"""Services for the ORLEN Paczka parcel tracker integration.

`orlen_paczka.track_parcel` / `orlen_paczka.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single ORLEN Paczka hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("ORLEN Paczka is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the ORLEN Paczka services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if tracking_code.startswith("AD"):
            raise ServiceValidationError(
                "Allegro Delivery codes starting with AD are tracked in Allegro, "
                "not ORLEN Paczka"
            )
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid ORLEN Paczka tracking code"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the ORLEN Paczka services (single-entry integration, so on unload)."""
