# Automatic parcel tracking from e-mail (IMAP → ORLEN Paczka)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the ORLEN Paczka tracking code, and register it with `orlen_paczka.track_parcel` — fully automatic, no extra custom component required.

ORLEN Paczka is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `orlen_paczka.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ orlen_paczka.track_parcel
                                     │
...
- This integration, with the `orlen_paczka.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9])(?:[23][0-9]{12}|[0-9]{10}|[0-9]{7})(?![0-9])
```

The pattern covers ORLEN's consumer-site validation: 13 digits beginning with `2` or `3`, plus exactly 7 or 10 digits. It puts the 13-digit alternative first, so a valid code is not accidentally captured as its final 10 digits. The boundaries avoid matching a code embedded in a longer order number.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\b…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9])` lookarounds instead of `\b`, `[0-9]` instead of `\d`.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@orlen_paczkashipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: 2101234567890","initial":true,"folder":"INBOX","username":"test"}'
```

Then `orlen_paczka.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
