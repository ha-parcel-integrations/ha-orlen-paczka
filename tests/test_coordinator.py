"""Tests for the ORLEN Paczka coordinator: fetching, caching and events.
...
from custom_components.orlen_paczka.api import ORLENPaczkaApiError
...
from custom_components.orlen_paczka.coordinator import ORLENPaczkaCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["history"] = sample["history"][:4]
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = ORLENPaczkaApiError("HTTP 500")
...
async def test_first_refresh_fires_nothing(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = pickup_sample()
    coordinator = ORLENPaczkaCoordinator(hass, client, entry)
...
async def test_carrier_never_emits_delivery_time_changed_event(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = ORLENPaczkaCoordinator(hass, client, entry)
