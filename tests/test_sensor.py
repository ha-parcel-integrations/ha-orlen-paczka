"""Tests for ORLEN Paczka sensor property logic."""
...
from custom_components.orlen_paczka.sensor import (
    ...
    ORLENPaczkaNextDeliverySensor,
    ORLENPaczkaParcelSensor,
)
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = ORLENPaczkaParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = ORLENPaczkaParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = ORLENPaczkaNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = ORLENPaczkaNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = ORLENPaczkaDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = ORLENPaczkaLastUpdateSensor(coordinator, _entry())
